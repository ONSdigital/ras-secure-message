...
class PartyService:
    ...
    def get_business_details(ru):
        ...
        logger.debug('party get business details result => {} {} : {}'.format(party_data.status_code, party_data.reason, party_data.text))
    ...
    def get_user_details(uuid):
        try:
            ...
            party_data = requests.get(url, verify=False)
            logger.debug('party get user details result => {} {} : {}'.format(party_data.status_code, party_data.reason, party_data.text))
            party_text = json.loads(party_data.text)
            return Response(response=json.dumps(party_text), status=200, mimetype="text/html")
        except KeyError:
