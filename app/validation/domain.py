...
class MessageSchema(Schema):
    ...
    def validate_to_from_not_equal(self, data):
        if 'msg_to' in data.keys() and 'msg_from' in data.keys() and data['msg_to'][0] == data['msg_from']:
            logger.debug('Message to and message from cannot be the same', message_to=data['msg_to'][0], message_from=data['msg_from'])
    ...
    def validate_to(self, msg_to):
        for item in msg_to:
            ...
            if msg_to != constants.BRES_USER and not User.is_valid_user(item):
                logger.debug('Not a valid user', user=item)
    ...
    def validate_from(self, msg_from):
        ...
        if g.user.is_internal and msg_from != constants.BRES_USER:
            logger.debug('Not authorised to send a message on behalf of user or work group', fessage_from=msg_from)
    ...
    def validate_not_present(data, field_name):
        if field_name in data.keys():
            logger.debug('Field cannon be set', field_name=field_name)
    ...
    def validate_non_zero_field_length(self, field_name, length, max_field_len):
        if length <= 0:
            logger.debug('Field not populated', field_name=field_name)
...
class DraftSchema(Schema):
    ...
    def validate_from(self, msg_from):
        ...
        if g.user.is_internal and msg_from != constants.BRES_USER:
            logger.debug('Not authorised to save a draft on behalf of user or work group', fessage_from=msg_from)
