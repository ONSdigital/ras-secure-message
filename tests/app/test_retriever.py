    ...
    def populate_database(self, no_of_messages=0, single=True, add_reply=False, add_draft=False, multiple_users=False):
        ...
        for _ in range(no_of_messages):
            ...
            if single:
                ...
                self.add_event(event=Events.SENT.value, msg_id=msg_id, date_time=datetime(year, month, day))

            if add_reply:
                self.del_status(label="UNREAD", msg_id=msg_id, actor=constants.BRES_USER)
                self.add_event(event=Events.READ.value, msg_id=msg_id, date_time=datetime(year, month, day + 1))
                ...
                self.add_event(event=Events.SENT.value, msg_id=msg_id, date_time=datetime(year, month, day + 1))
            ...
            if add_draft:
                ...
                self.add_event(event=Events.DRAFT_SAVED.value, msg_id=msg_id, date_time=datetime(year, month, day))
    ...
    def create_threads(self, no_of_threads=1, add_internal_draft=False, add_respondent_draft=False):
        ...
        for _ in range(no_of_threads):
            ...
            if add_internal_draft:  # adds draft from internal
                ...
                self.add_status(label="DRAFT_INBOX", msg_id=msg_id, actor="0a7ad740-10d5-4ecb-b7ca-3c0384afb882")
                self.add_status(label="DRAFT", msg_id=msg_id, actor=constants.BRES_USER)
                self.add_event(event=Events.DRAFT_SAVED.value, msg_id=msg_id, date_time=datetime(year, month, day + 2))
            ...
            if add_respondent_draft:  # adds draft from respondent
                ...
                self.add_event(event=Events.READ.value, msg_id=last_msg_id, date_time=datetime(year, month, day + 1))
    ...
    def test_read_date_returned_for_message(self):
        ...
        with self.engine.connect() as con:
            query = "SELECT securemessage.secure_message.msg_id FROM securemessage.secure_message " \
                    ...
                    "WHERE securemessage.events.event = '" + Events.READ.valueRead + "' LIMIT 1"
            query_x = con.execute(query)
            names = []
            for row in query_x:
                names.append(row[0])
