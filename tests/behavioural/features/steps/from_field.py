...
@given("the from is set to internal user")
@when("the from is set to internal user")
def step_impl_the_msg_from_is_set_to_internal_user(context):
    """set the from to the internal user as set in the helper """
    step_impl_the_msg_from_is_set_to(context, context.bdd_helper.__INTERNAL_USER_GROUP)
