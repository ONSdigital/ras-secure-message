...
class PartyService:
    ...
    def does_user_have_claim(user_id: str, business_id: str, survey_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(
                "Failed to validate claim data with party service",
                status_code=response.status_code,
                text=response.text,
        ...
        logger.info(
            "Party claim data successfully retrieved ",
            respondent_id=user_id,
            business_id=business_id,
    ...
    def _get_user_details_from_party_service(uuids: list[str]):
        ...
        response = requests.get(
            f"{current_app.config['PARTY_URL']}/party-api/v1/respondents",
            auth=current_app.config["BASIC_AUTH"],
            verify=False,
            params=params,
        )
        try:
            ...
            logger.exception(
                "Failed to retrieve data from party service",
                status_code=response.status_code,
                text=response.text,
                uuids=uuids,
            )
